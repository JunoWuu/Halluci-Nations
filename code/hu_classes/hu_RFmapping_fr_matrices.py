...
class hu_RFmapping_fr_matrices:
    ...
    def gen_rf_maps(self):
        ...
        coords_dic = {'x_coords':self.rf_x_coords,
                      'y_coords':self.rf_y_coords}
        savepath2 = os.path.join(self.main_path,'receptive_map_coords.pkl') 
        with open(savepath2, 'wb') as f:
            pickle.dump(coords_dic, f)
        return 
    ...
    def get_tunning_curves(self):
        ...
        savepath = os.path.join(self.main_path,'orientation_coords.pkl')
        ori_coords = list(subselected_data.keys())
        with open(savepath, 'wb') as f:
            pickle.dump(ori_coords, f)
        savepath = os.path.join(self.main_path,'tunning.pkl') 
        with open(savepath, 'wb') as f:
